...
def add_account(account_name, initial_balance):
    ...
    cursor.execute('INSERT INTO bank_account (account_name, balance) VALUES (?, ?)', (account_name, initial_balance))
...
def send_email_alert(subject, message, to_email):
    ...
    password = 'ptti eyjp dpdt qxst'             # Replace with sender email password (use an app password if using Gmail)
    ...
    try:
        ...
        server.sendmail(from_email, to_email, msg = "Anil sent") # Corrected line for sending the email
...
#create_database()
add_account("Anil's Account", 1000.0)
